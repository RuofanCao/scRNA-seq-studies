...
###### step1: 导入数据 ######   
dir='GSE184242_RAW/scRNA2/' 
samples=list.files('GSE184242_RAW/scRNA2/','^GSM')
fs
library(tidyverse)
...
if(T){
  ...
  # 
  celltype[celltype$ClusterID %in% c( 4 ),2]='endo'
  celltype[celltype$ClusterID %in% c( 3 ),2]='fibro'
  #celltype[celltype$ClusterID %in% c( 5 ),2]='SMC'    
  # # ductal ROIs (MUC5B and MUC20).
  # celltype[celltype$ClusterID %in% c( 4 ),2]='ductal' 
  # # acinar ROIs (CEL, CPA1, CELA2B, and PRSS3)
  # celltype[celltype$ClusterID %in% c( 0 ),2]='acinar' 
  # celltype[celltype$ClusterID %in% c( 2 ),2]='epi' 
  celltype[celltype$ClusterID %in% c( 7 ),2]='cycle' 
  # #  "CHGB", "CHGA", # enteroendocrine cells
  # celltype[celltype$ClusterID %in% c( 11 ),2]='endocrine' 
...
# 过滤无法命名的 
sce.all.int = sce.all.int[,!sce.all.int$celltype %in% 0:100]
table(sce.all.int$celltype)
...
# 就可以运行下面的代码
sce.all.int
sce.all.int$celltype[sce.all.int$celltype=='']='unknown'
table(sce.all.int$celltype)
sce.all.int=sce.all.int[,sce.all.int$celltype !='unknown']
...
cg=c('PTMA','CD83','CLEC4A','HCAR2','CD14','USP10','NCF1','S100P','MMP9','')
cg
p1 <- DotPlot(sce.all.int, features = unique(str_to_upper(cg)),
              assay='RNA' ,group.by = 'RNA_snn_res.0.1'  )  + coord_flip() # +th

p1
p_umap=DimPlot(sce.all.int, reduction = "umap"
               ,group.by = 'RNA_snn_res.0.1' ,raster = F,
               label = T,repel = T) 
p_umap 
p1 + p_umap

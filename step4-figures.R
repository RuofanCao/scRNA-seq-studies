...
## 1. 载入，并且理解数据 ---- 
sce.all = readRDS('../2-harmony_immune/sce.all_int.rds')
sp='human' 
load('../phe_new.Rdata') 
phe = phe_new
...
ord = c( "CD4T"   ,       "B"      ,       "Myeloid"    ,   "Proliferative",
         "CD8T"    ,      "Plasma"     ,   "NK"   )
...
sce$group = phe$response
table(sce$group) 
#注意颜色的修改，看个人喜欢什么色系
plot1 <- DimPlot(sce, reduction = "umap", group.by = "RNA_snn_res.0.3",label = T,repel = T)+  
...
plot2 <- DimPlot(sce, reduction = "umap", group.by = "celltype",label = F,cols = color,repel = T) +
  theme(legend.key.size = unit(0.02, 'cm')) + theme(plot.title = element_text(size=12),
                                                    ...
                                                    axis.text = element_text(size=10),
                                                    legend.text = element_text(size=10))
plot3 <- DimPlot(sce, reduction = "umap", group.by = "group",label = T,cols = sample(color,10),repel = T) +
...
fig1
ggsave(fig1,filename = "fig1.pdf",units = "cm",width = 22,height = 12)
...
plot5
ggsave(plot5,filename = "fig2-2.pdf",units = "cm",
       width = 20,height = 10)
...
ggsave(filename = "fig4-top10-marker_pheatmap.pdf",units = "cm",
       width = 25,height = 30)
...
table(phe$response)
#首先单独提取耐药和反应的细胞亚群，分别可视化密度图
choose_cellIds = rownames(phe)[phe$response %in% c('Responder') ]
response_sce = sce[,choose_cellIds]
choose_cellIds=rownames(phe)[phe$response %in% c('Non-responder') ]
resistant_sce = sce[,choose_cellIds]

####CD4发挥免疫作用的一系列基因list，可以添加
plot_density(response_sce, "CD4")
plot_density(response_sce, "CD8A")
####CD8发挥免疫作用的一系列基因list
#也可以用Featureplot，右上角可以黄色越深的地方两个基因叠加越多
FeaturePlot(resistant_sce, features = c('CD4','CD8A'),

...
#### txt/csv/tsv data 读取###
# 一般下载下来的都是tar结尾的压缩文件
#untar("GSE155698_RAW.tar",exdir = "GSE155698_RAW")
dir='GSE155698_RAW' 
fs=list.files('GSE155698_RAW/','^GSM')
fs
# 使用grep筛选出不包含"PBMC"的文件名
setwd('GSE155698_RAW/')
files_to_remove <- grep("PBMC", fs, value = TRUE)
...
#     # x = unique(samples)[1]
#     y=fs[grepl(x,fs)]
#     folder=paste0("GSE155698/GSE155698_RAW/", paste(str_split(y[1],'_',simplify = T)[,1:2], collapse = "_"))
#     dir.create(folder,recursive = T)
#     #为每个样本创建子文件夹
#     file.rename(paste0("GSE155698/GSE155698_RAW/",y[1]),file.path(folder,"barcodes.tsv.gz"))
#     #重命名文件，并移动到相应的子文件夹里
#     file.rename(paste0("GSE155698/GSE155698_RAW/",y[2]),file.path(folder,"features.tsv.gz"))
#     file.rename(paste0("GSE155698/GSE155698_RAW/",y[3]),file.path(folder,"matrix.mtx.gz"))
#   })
# }
...
if(length(fs) > 0) {
  lapply(unique(samples), function(x) {
    #x = unique(samples)[1]
    y <- fs[grepl(x, fs)]
    folder <- paste0("GSE155698_data/", paste(str_split(y[1], '_', simplify = TRUE)[, 1:3], collapse = "_"))
    dir.create(folder, recursive = TRUE)
    untar(y[1], exdir = folder)
    subfolder <- list.dirs(folder, full.names = TRUE, recursive = FALSE)[1]
    subsubfolder = list.dirs(subfolder, full.names = TRUE, recursive = FALSE)[1]
    files_in_subfolder <- list.files(subsubfolder, full.names = TRUE)
    
    # 移动三个文件到第一层级
    for (file_path in files_in_subfolder) {
      file.rename(file_path, file.path(folder, basename(file_path)))
    }
...
###检查数据，可以直接在linux中使用tree来查看层级结构
setwd('../')
dir='GSE155698_RAW/GSE155698_data/' 
...
Idents(sce.all.int)
table(sce.all.int$RNA_snn_res.0.5) 
###设置为RNA_snn_res.0.3继续后续的单细胞亚群注释
sel.clust = "RNA_snn_res.0.5"
sce.all.int <- SetIdent(sce.all.int, value = sel.clust)
table(sce.all.int@active.ident) 
table(sce.all.int$RNA_snn_res.0.5)
...
genes_to_check <- c("KRT19", "EPCAM", "FXYD2", "ANXA4", "PRSS1", "CLPS", "PCSK1N", "TTR", "CTSS",
                              "FCN1", "CD68", "APOE", "CSF3R", "FCGR3B", "IRF8", "IRF7", "TPSAB1", 
                              "CPA3", "CD3D", "IL7R", "CD8A", "CCL5", "TIGIT", "FOXP3", "NKG7", 
                              "GZMA", "CD79A", "MS4A1", 'IGLL5','IGJ',
                              "STMN1", "MKI67", "PDGFRB", "TAGLN", "COL1A1", "COL1A2", "VWF", "CLDN5")
library(stringr)  
...
#结合check-by-0.1文件夹中的marker基因和上述genes_to_check的dotplot
if(T){
  sce.all.int
  celltype=data.frame(ClusterID=0:23 ,
                      celltype=0:23 ) 
  #定义细胞亚群        
  celltype[celltype$ClusterID %in% c( 0,9,11 ),2]='Epithelial'
  celltype[celltype$ClusterID %in% c( 18  ),2]='Ductal'
  celltype[celltype$ClusterID %in% c(6),2]='Acinar'
  celltype[celltype$ClusterID %in% c(21 ),2]='Endocrine'
  celltype[celltype$ClusterID %in% c( 7 ),2]='Monocytes'
  celltype[celltype$ClusterID %in% c( 4,5 ),2]='Macrophages'
  celltype[celltype$ClusterID %in% c(1,17 ),2]='Neutrophils'
  ###可以看一下每个亚群的特异性基因表达top3
  #如果还不确定，可以走一下自动注释大致对应一下
  celltype[celltype$ClusterID %in% c( 22 ),2]='DC'  
  celltype[celltype$ClusterID %in% c( 8),2]='Mast'  
  celltype[celltype$ClusterID %in% c( 2,20 ),2]='CD4T'
  celltype[celltype$ClusterID %in% c( 19 ),2]='CD8T' 
  celltype[celltype$ClusterID %in% c(  ),2]='Tregs'  
  celltype[celltype$ClusterID %in% c( 3),2]='NK'  
  celltype[celltype$ClusterID %in% c( 15 ),2]='B'
  celltype[celltype$ClusterID %in% c(12,23 ),2]='Plasma' 
  celltype[celltype$ClusterID %in% c( 13),2]='Proliferative'  
  celltype[celltype$ClusterID %in% c( 10,14 ),2]='Fibroblasts'
  celltype[celltype$ClusterID %in% c( 16 ),2]='Endothelial' 
  ...
  ###上面选择的是RNA_snn_res.0.5，如果是别的resolution，是需要修改的
...
umap1=DimPlot(sce.all.int, reduction = "umap",
              # split.by = "orig.ident",
              group.by = "RNA_snn_res.0.5",label = T,cols = color)
umap1
